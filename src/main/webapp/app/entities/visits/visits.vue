<template>
  <div>
    <h2 id="page-heading" data-cy="VisitsHeading">
      <span v-text="$t('petclinicApp.visits.home.title')" id="visits-heading">Visits</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('petclinicApp.visits.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'VisitsCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('petclinicApp.visits.home.createLabel')"> Create a new Visits </span>
          </button>
        </router-link>
      </div>
    </h2>
        ...
        <form name="searchForm" class="form-inline" v-on:submit.prevent="search(currentSearch)">
          <div class="input-group w-100 mt-3">
            <input
              ...
              name="currentSearch"
              id="currentSearch"
              v-bind:placeholder="$t('petclinicApp.visits.home.search')"
    ...
    <div class="alert alert-warning" v-if="!isFetching && visits && visits.length === 0">
      <span v-text="$t('petclinicApp.visits.home.notFound')">No visits found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="visits">
        ...
        <tbody>
          <tr v-for="visits in visits" :key="visits.id" data-cy="entityTable">
            ...
            <td>{{ visits.visitdate ? $d(Date.parse(visits.visitdate), 'short') : '' }}</td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="petclinicApp.visits.delete.question" data-cy="visitsDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-visits-heading" v-text="$t('petclinicApp.visits.delete.question', { id: removeId })">
